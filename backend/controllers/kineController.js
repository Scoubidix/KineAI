...
const createKine = async (req, res) => {
  ...
  try {
    ...
    // Kine.email est @unique : l'email du token est déjà porté par une autre ligne (email
    // modifié via le profil, ou ligne survivante d'un compte Firebase supprimé). Erreur
    // explicite plutôt qu'un 500 générique, pour que le front et le support la reconnaissent.
    if (err?.code === 'P2002') {
      logger.warn("🚫 Création kiné refusée : email déjà enregistré - UID:", sanitizeUID(uid));
      return res.status(409).json({ success: false, error: 'Un compte existe déjà avec cette adresse email.', code: 'EMAIL_ALREADY_REGISTERED' });
    }
    logger.error("❌ Erreur création kiné:", err.message);
    return res.status(500).json({ success: false, error: 'Erreur serveur lors de la création du kiné.', code: 'INTERNAL_ERROR' });
  }
};
