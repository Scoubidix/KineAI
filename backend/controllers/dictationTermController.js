...
/** PATCH /api/admin/dictation-terms/:id — corrige une paire (rogner une sélection trop large) */
exports.adminEdit = async (req, res) => {
  try {
    ...
    const term = await dictationTermService.adminEdit({ id, heard: req.body.heard, expected: req.body.expected });
    return res.json({ success: true, term });
  } catch (err) {
    if (err?.status && err?.code) return res.status(err.status).json({ success: false, error: err.message, code: err.code });
    if (err?.code === 'P2025') return res.status(404).json({ success: false, error: 'Terme introuvable', code: 'TERM_NOT_FOUND' });
    logger.error(`Vocabulaire signalé : correction en échec (${err?.message})`);
