...
function parseReport(content) {
  ...
  const sections = {};
  for (const k of SECTION_KEYS) sections[k] = String(result.data.sections[k] || '').replace(/\s+/g, ' ').trim();
  return sections;
}
 ...
 * Le dialogue est normalisé lettres → chiffres avant comparaison : les locuteurs parlent en toutes
 * lettres (« vingt-deux »), le modèle écrit toujours en chiffres dans les sections.
 */
function guardReport(transcript, sections) {
  const allowed = new Set(numbersIn(wordsToDigits(transcript)));
